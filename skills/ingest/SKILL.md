...
`Read, Write, Edit, Bash, Grep, Glob`. A typical ingest touches 5–15 vault
pages and reads the source in full; its output would flood a caller's context
if run inline.
...
`gw ingest` classifies the source kind from its content, not from a folder path. `source_kind` is a closed enum — see `.gw/schema/Source.schema.json` for the authoritative list: `spec`, `article`, `ticket`, `skill`, `doc`, `transcript`, `code-review`. (`skill` classifies but has no guidance-page flow of its own; it ingests like any other source.)
...
Follow `../graph-works/references/ingest-workflow.md`. Summary:
...
- **Auto-discovery is `.md` only.** Direct `/gw:ingest <path>` works for any format `gw ingest` understands.
