...
# the first screen. Session files persist per the fallback chain below;
# --project-dir /path/to/project overrides it and enables same-port restart.
bash "${CLAUDE_PLUGIN_ROOT}/skills/brainstorming/scripts/start-server.sh" --open

# Returns (no --project-dir, so this reflects the fallback chain — here the
# repo-root .brainstorming/ tier; see "Note" below for the other tiers):
# {"type":"server-started","port":52341,
#  "url":"http://localhost:52341/?key=ab12…",
#  "screen_dir":"<repo-root>/.brainstorming/12345-1706000000/content",
#  "state_dir":"<repo-root>/.brainstorming/12345-1706000000/state"}
```
...
**Note:** Session files are persisted where the workspace resolves — `<workspace>/brainstorm/` when a graph-works workspace is found, else `.brainstorming/` at the enclosing git repo root, else `/tmp/brainstorm-<session>` (ephemeral). `--project-dir <path>` overrides the chain and stores under `<path>/.brainstorming/`. Remind the user to add `.brainstorming/` to `.gitignore` if it's not already there and a persistent tier resolved.
...
bash "${CLAUDE_PLUGIN_ROOT}/skills/brainstorming/scripts/start-server.sh" --open --foreground
```
...
Persistence follows the same fallback chain used at startup: the `<workspace>/brainstorm/` and `.brainstorming/` (repo root or `--project-dir` override) tiers persist, so mockup files survive for later reference. Only the `/tmp/brainstorm-<session>` tier is ephemeral and gets deleted on stop.
