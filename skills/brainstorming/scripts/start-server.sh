...
#                         <path>/.brainstorming. Without it, the session
#                         dir is resolved (graph-works workspace -> repo -> /tmp);
...
if [[ -n "$PROJECT_DIR" ]]; then
  ...
  # These MUST track SESSION_DIR's directory: upstream introduced them hard-coded
  # to the pre-rename brainstorm path, which the rename above would otherwise strand.
  ...
  #   1. graph-works workspace resolves -> <workspace>/brainstorm/
  #   2. else inside a git repo        -> <repo>/.brainstorming/  (persists, as before)
  ...
  if [[ -n "$WORKSPACE" ]]; then
    ...
    SESSION_DIR="${REPO_ROOT}/.brainstorming/${SESSION_ID}"
  else
